...
public class ConfigData {
	...
	public void addRoot(ASMData asmData) {
		...
		if (!containerMap.containsKey(packageName)) {
			DPLogger.info("Discovered config controller inside: " + packageName);

			File defaultConfig = getConfigForPackage(annotationInfo);

			DPLogger.info("Setting config: " + defaultConfig);

			containerMap.put(packageName, new ConfigContainer(defaultConfig));
		} else {
			DPLogger.warning("Config controller discovered in the same root: " + packageName);
			DPLogger.warning("THIS IS AN ERROR! Ignoring " + asmData.getClassName());
		}
	}

	public void processIConfigRegistrar(IConfigRegistry registry) {
		DPLogger.info("Registering custom annotations for " + modContainer.getModId());
	...
	/**
	 * Go from largest to smallest, that way when the childClasses get processed
	 * it can pull them out of the pool, without them already belonging to a more
	 * generic package.
	 */
