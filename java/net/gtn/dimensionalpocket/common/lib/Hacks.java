 ...
 *
 * @author NPException
 * @deprecateded This class is deprecated to further indicate it's temporary
 *               nature
 *
 */
public class Hacks {

	public static CoordSet asChunkOffset(CoordSet cs) {
		cs.x = cs.x < 0 ? ((cs.x % 16) + 16) & 15 : (cs.x % 16);
		cs.y = cs.y < 0 ? ((cs.y % 16) + 16) & 15 : (cs.y % 16);
		cs.z = cs.z < 0 ? ((cs.z % 16) + 16) & 15 : (cs.z % 16);
		return cs;
	}

	public static CoordSet toChunkOffset(CoordSet cs) {
		return asChunkOffset(cs.copy());
	}
